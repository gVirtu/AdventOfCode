...
class State(object):
    ...
    def shouldBuild(self, blueprint, resource):
        remainingTime = TIME_LIMIT - self.time
        if resource == ORE and blueprint.costs[resource][ORE] >= remainingTime:
            return False
        return True
    ...
    def canBest(self, bestGeodes):
        remainingTime = TIME_LIMIT - self.time
        currentGeodes = self.resources[GEODE]

        # Assume a geode robot will be created at every step until the end
        currentProduction = self.robots[GEODE]
        endProduction = self.robots[GEODE] + remainingTime
        bestGeodeProduction = (
            (remainingTime + 1) * (currentProduction + endProduction) / 2
        )

        return currentGeodes + bestGeodeProduction > bestGeodes
...
for blueprintId, blueprint in enumerate(blueprints[:3], 1):
    bestGeodes = 0
    ...
    while queue:
        ...
        if not state.canBest(bestGeodes):
            continue
        ...
        for i in range(RESOURCE_TYPE_COUNT):
            canBuildRobot[i] = state.canBuild(blueprint, i) and state.shouldBuild(
                blueprint, i
            )
